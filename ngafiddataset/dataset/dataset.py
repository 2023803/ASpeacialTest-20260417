...
class NGAFID_Dataset_Downloader:
    ...
    def _all_paths_exist(paths):
        return paths is not None and all(os.path.exists(path) for path in paths)

    @classmethod
    def download(cls, name : str, destination:str = '', extract = True, required_files = None):
        ...
        if cls._all_paths_exist(required_files):
            logger.info('Using existing extracted dataset from {}', os.path.join(destination, name))
            return name, destination

        if not os.path.exists(output):
            logger.info('Local archive not found at {}. Attempting download from source.', output)
            try:
                gdown.download(url, output, quiet=False)
            except Exception as e:
                raise RuntimeError(
                    "Failed to download dataset archive automatically. "
                    "Please manually place %s in %s or extract it to %s before rerunning." % (
                        "%s.tar.gz" % name,
                        destination,
        ...
        if extract and not cls._all_paths_exist(required_files):
            logger.info('Extracting archive from {}', output)
            with tarfile.open(output) as archive:
                archive.extractall(destination)

        if required_files is not None and not cls._all_paths_exist(required_files):
            raise FileNotFoundError(
                "Dataset files were not found after extraction. Expected files: %s" % required_files
            )
...
class NGAFID_Dataset_Manager(NGAFID_Dataset_Downloader):
    ...
    def __init__(self, name: str, destination:str = '',  max_length = 4096, extract = True, **kwargs):
        ...
        if name == 'all_flights':
            logger.info('Downloading and extracting Parquet Files to {}\\one_parq. Please open them using dask dataframes', destination)
            ...
            self.download(name, destination, extract, required_files=list(self.files.values()))
